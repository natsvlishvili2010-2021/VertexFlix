  ...
  if (!show) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-24 text-center">
        <h1 className="text-2xl text-white font-bold mb-4">TV Show not found</h1>
    ...
    <div className="bg-[#050505] min-h-screen text-white">
                ...
                <div className="mt-6 space-y-3">
                  <div className="bg-white/5 border border-white/10 backdrop-blur-md rounded-xl p-4 flex flex-col items-center justify-center gap-1">
                    <div className="flex items-center gap-2">
                      <span className="text-2xl font-black text-accent">{show.vote_average.toFixed(1)}</span>
                    ...
                    <span className="text-[10px] text-white/30 font-medium">{show.vote_count.toLocaleString()} votes</span>
                  </div>
      ...
      {show.seasons && show.seasons.length > 0 && (
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
          <h2 className="text-white text-lg sm:text-xl font-bold mb-6">Episodes</h2>
          <EpisodeGrid showId={id} seasons={show.seasons} onPlayEpisode={handlePlayEpisode} />
        </section>
      )}
      ...
      {show.similar?.results?.length > 0 && (
        <MovieCarousel title="Similar Shows" items={show.similar.results} mediaType="tv" />
      )}

      {/* Recommendations */}
      {show.recommendations?.results?.length > 0 && (
        <MovieCarousel title="Recommended" items={show.recommendations.results} mediaType="tv" />
      )}
    </div>
  );
}
